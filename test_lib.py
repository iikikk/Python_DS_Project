...
sample_data = pd.DataFrame(
    {"Year": [2020, 2021, 2022, 2023], "Incidents": [5, 10, 15, 20]}
)
...
def test_generate_summary(mock_data):
    ...
    assert (
        "head" in summary
        and "dtypes" in summary
        and "describe" in summary
        and "year_stats" in summary
    )
    assert (
        len(summary["head"]) == 4
    )  # Checks if head returns the default number of rows
